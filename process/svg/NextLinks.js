...
class LN {
  ...
  append (ln) {
    if (this.id === ln.id) {
      ...
      var appendSuccessNum = !this.next.every(lnObj => !lnObj.append(ln.clone()));
...
function mergeLn (lnArr) {
  ...
  while (1) {
    result = init.reduce((pre, next) => {
      ...
      if (pre.length) {
        var isAppend = !pre.every(ln => !ln.append(next));
        if (isAppend) {
          return pre;
        } else {
...
function findType (nodeTree, type) {
  var r = nodeTree.some(o => o.type === type);
  if (!r) {
...
function buildTree (connections, allLn, isRerverse) {
  ...
  connections.forEach(({from, to, mirror}, i) => {
    ...
    var isAppend = !nodeTree.every(ln => {
...
function findNodeById (tree, id) {
  var ln = tree.filter(ln => String(ln.id) === String(id))[0];
  if (!ln) {
    ln = tree.map(ln => {
...
export default class NextLink {
  constructor (connections, allNodes) {
    var allLn = allNodes.map(o => new LN(o));
    var nodeTree = mergeLn(buildTree(connections, allLn)); //
    var nodeTreeReverse = mergeLn(buildTree(connections, allLn, true)); // 方向相反
  ...
  findTypeBefore (prevType, endType) {
    // console.log('==>', endType);
    var r = this.singlenodeTree.filter(startNode => {
