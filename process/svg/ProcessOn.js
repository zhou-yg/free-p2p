...
const NODE_STATE_ICON = [
  require('@/../static/img/flow/node-state-success.png'),
  require('@/../static/img/flow/node-state-warning.png')
]
...
Raphael.fn.connection = function (obj1, obj2, event) {
  ...
  let x2 = ((x4 + x1) / 2).toFixed(3)
  let y2 = y1.toFixed(3)

  let textX = y1 === y4 ? (x1 + x4) / 2 : (parseFloat(x2) + x4) / 2
  let textY = y4 - 10
...
function move (dx, dy) {
  ...
  if (this.paper && this.paper.canvas) {
    w = this.paper.canvas.clientWidth
    h = this.paper.canvas.clientHeight
  }
  let att = {x: Math.min(Math.max(this.ox + dx, 5), w - 65), y: Math.min(Math.max(this.oy + dy, 5), h - 85)}
  if (this.type === 'image' || this.type === 'rect') {
...
class Node {
  constructor ({id, type, name, x, y, w = 48, h = 48, formData, onClick, onDoubleClickNode, origin}) {
    ...
    // 包含的节点
    this.nodeKeys = ['rNode', 'baseCenterNode', 'glowCache', 'centerNode', 'connectionNodes']
    // 预设值
  ...
  markState (r, state) {
    ...
    this.stateNode = r.image(stateIcon, x + w - bgR * centerCof / 1.2, h + y - bgR * centerCof / 1.2, bgR * centerCof, bgR * centerCof).attr({
      fill: '#fff'
    })
  }
  remove () {
...
class LineNode extends Node {
  ...
  generate (r, {path, attr, eventOffset}) {
    this.rNode = r.path(path).attr(attr)
    this.baseCenterNode = this.rNode
...
export class ImageNode extends Node {
  ...
  generate (r) {
    ...
    this.baseCenterNode = r.circle(this.x + this.w / 2, this.y + this.h / 2, (this.w - centerPd * 2 + this.centerBd) / 2)
    this.baseCenterNode.attr({
      'stroke-width': 0,
      'fill': this.fill,
      'opacity': 0
    ...
    this.rNode = r.circle(this.x + this.w / 2, this.y + this.h / 2, (this.w - centerPd * 2) / 2)
    this.rNode.attr({
      'stroke-width': 0,
      'fill': this.fill
    })
...
export class ProcessOn {
  ...
  constructor ({id, w = 640, h = 480, disabledEdit, onClickNode, onDoubleClickNode, onStateChange, onNodeConnect}) {
    window.PO = this
    ...
    this.shapes = new ProcessShapesManager()
    this.connections = []
  ...
  dragNodeEvent (node) {
    let isDragger
    node.centerNode.drag((dx, dy, _, __, e) => {
      isDragger = true
      // 老式连线的代码
      // if (this.editLineMode) {
      //   return;
      ...
      raf(() => {
        const moveAttr = move.call(node.rNode, dx, dy)
        node.moveTo(moveAttr)
      ...
      dragger.call(node.rNode)
      // 老式连线的代码
      // if (this.editLineMode) {
      //   this.startConnectLine(node);
      // }
    }, (e) => {
      if (isDragger) {
  ...
  addNode (node, prev) {
    ...
    node.generate(this.r)
    this.dragNodeEvent(node)
    this.connectionNodeEvent(node)
